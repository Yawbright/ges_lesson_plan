...
async function extractPdfText(buffer) {
  ...
  try {
    ...
    return await execFileText('python', ['-c', pythonCode]);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
}
...
function normalizeClassLabel(classLevel) {
  ...
  if (!value) return '';
  const match = value.match(/\b([789])\b/);
...
function detectClassFromLine(line) {
  ...
  const match = value.match(/\b(?:basic|b|jhs)\s*([789])\b/);
  return match ? `b${match[1]}` : '';
}

function detectAvailableClassLevels(text) {
  const matches = String(text || '').match(/\b(?:basic|b|jhs)\s*([789])\b/gi) || [];
  const seen = new Set();
  const ordered = [];
...
function detectTermFromLine(line) {
  ...
  if (
    /\b(term|semester)\s*1\b/.test(value) ||
    /\b1st\s+(term|semester)\b/.test(value) ||
    /\bfirst\s+(term|semester)\b/.test(value) ||
    /\b(term|semester)\s*one\b/.test(value) ||
    /\b(term|semester)\s*i\b/.test(value) ||
    /\bterm\s*-\s*1\b/.test(value) ||
    /\bterm\s*:\s*1\b/.test(value)
  ) {
    return 'term 1';
  }
  ...
  if (
    /\b(term|semester)\s*2\b/.test(value) ||
    /\b2nd\s+(term|semester)\b/.test(value) ||
    /\bsecond\s+(term|semester)\b/.test(value) ||
    /\b(term|semester)\s*two\b/.test(value) ||
    /\b(term|semester)\s*ii\b/.test(value) ||
    /\bterm\s*-\s*2\b/.test(value) ||
    /\bterm\s*:\s*2\b/.test(value)
  ) {
    return 'term 2';
  }
  ...
  if (
    /\b(term|semester)\s*3\b/.test(value) ||
    /\b3rd\s+(term|semester)\b/.test(value) ||
    /\bthird\s+(term|semester)\b/.test(value) ||
    /\b(term|semester)\s*three\b/.test(value) ||
    /\b(term|semester)\s*iii\b/.test(value) ||
    /\bterm\s*-\s*3\b/.test(value) ||
    /\bterm\s*:\s*3\b/.test(value)
  ) {
    return 'term 3';
  }
...
function scoreTermMarker(line, normalizedTerm) {
  ...
  if (countDistinctTermMentions(value) > 1) {
    score -= 5;
  }
  ...
  if (detectTermFromLine(value) === normalizedTerm) {
    score += 2;
  }
...
function isWeekLikeLine(line) {
  const value = cleanText(line).toLowerCase();
  return (
    /\b(week|wk|w\/k)\s*[:.\-]?\s*\d{1,2}\b/.test(value) ||
    /^\d{1,2}\s+[a-z]/.test(value)
  );
}
...
function detectUploadedSchemeMetadata(text) {
  ...
  const subjectPatterns = [
    ...
    {
      pattern:
        /\bcomputing\b|\bict\b|\binformation and communication technology\b|\bcomputer studies\b/i,
      subject: 'Computing',
    },
    { pattern: /\benglish language\b|\benglish\b/i, subject: 'English' },
    { pattern: /\bsocial studies\b/i, subject: 'Social Studies' },
  ];

  const subjectMatch = subjectPatterns.find((entry) => entry.pattern.test(lower));
  const classMatch = compact.match(/\b(?:basic|b|jhs)\s*([789])\b/i);
...
function reconcileParsedSchemeWithCurriculumBackend(input) {
  const curriculumWeeks = Array.isArray(input.curriculumYearHint)
    ? input.curriculumYearHint
    : [];
  ...
  if (!curriculumWeeks.length) {
    return {
      ...
      parserMetadata: {
        ...
        warnings: [
          'No mapped curriculum was available for this subject/class, so the uploaded scheme was kept as parsed.',
        ],
        confidence: 0.45,
      },
    };
  ...
  const reconciledWeeks = (input.scheme.weeks || []).map((week) => {
    ...
    if (!match) {
      warnings.push(
        `Week ${week.week} could not be matched confidently to the mapped curriculum and was kept mostly as uploaded.`
      );
      totalConfidence += 0.2;
      return {
        ...week,
        uploadedTopic: week.topic,
        matchedCurriculumTerm: undefined,
        matchConfidence: 0.2,
      };
    }
    ...
    if (normalizeSimpleTerm(match.week.sourceTerm) !== normalizeSimpleTerm(input.preferredTerm)) {
      warnings.push(
        `Week ${week.week} matched a curriculum topic usually placed in ${match.week.sourceTerm}, but the uploaded scheme keeps it in ${input.preferredTerm}.`
      );
    }
  ...
  const confidence = reconciledWeeks.length
    ? Number((totalConfidence / reconciledWeeks.length).toFixed(2))
    : 0;
...
function mergeEntriesBackend(uploadedWeek, curriculumWeek) {
  ...
  if (curriculumEntries.length) {
    return curriculumEntries.map((entry, index) => ({
      ...
      contentStandard:
        cleanText(entry?.contentStandard) || cleanText(uploadedEntries[index]?.contentStandard),
...
function findBestCurriculumMatchBackend(uploadedWeek, curriculumWeeks, usedCurriculumIndexes) {
  const uploadedTokens = tokenizeForMatch(
    [
      uploadedWeek.topic,
      uploadedWeek.strand,
      uploadedWeek.subStrand,
      uploadedWeek.contentStandard,
      uploadedWeek.indicator,
      ...(Array.isArray(uploadedWeek.entries)
        ? uploadedWeek.entries.flatMap((entry) => [
            entry?.topic,
            entry?.strand,
            entry?.subStrand,
  ...
  curriculumWeeks.forEach((week, index) => {
    ...
    const confidence =
      shared === 0
        ? 0
        : Number((shared / Math.max(4, uploadedTokens.size)).toFixed(2));
