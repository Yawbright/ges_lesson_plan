...
export function buildSocialStudiesLessonFocusGuidance(input: {
  subject: string;
  classLevel: string;
  week?: SchemeWeek;
  sessionIndex?: number;
  ...
  if (!isJhsSocialStudies(input.subject, input.classLevel) || !input.week) return undefined;
  ...
  const exemplarItems = uniqueStrings(candidateCodes)
    .flatMap((code) => socialStudiesExemplarsByIndicator[code]?.exemplars ?? []);
...
function getBestCodesForStandard(
  ...
  const candidates = Object.entries(socialStudiesExemplarsByIndicator)
...
function buildThreeLessonFocuses(exemplars: string[]): string[] {
  ...
  if (exemplars.length === 2) {
    return [
      exemplars[0],
      exemplars[1],
      `Consolidate and apply through discussion, presentation or community action: ${exemplars[0]} ${exemplars[1]}`,
    ];
  }

  return [
    exemplars[0],
    `Extend through guided inquiry and discussion: ${exemplars[0]}`,
    `Consolidate through presentation, reflection or community application: ${exemplars[0]}`,
  ];
}
...
function extractIndicatorCodes(value?: string): string[] {
  return value?.match(/B[789]\/JHS[123](?:\.\d+){4}/g) ?? [];
}
...
function isJhsSocialStudies(subject: string, classLevel: string): boolean {
  return /social\s*studies/i.test(subject) && ['B7', 'B8', 'B9'].includes(classLevel);
}
